...
export class AppApiChannel {
    ...
    // 获取系统盘临时文件存储位置
    static GET_APP_TEMP_DATA_PATH = StringUtils.joinPaths(this.channelsPrefix, 'getAppTempDataPath');
    // 获取应用日志文件存储位置
    static GET_APP_LOG_PATH = StringUtils.joinPaths(this.channelsPrefix, 'getAppLogPath');
    // 获取应用缓存存储位置
    static GET_APP_CACHE_PATH = StringUtils.joinPaths(this.channelsPrefix, 'getAppCachePath');

    // 打开给定文件或路径
    static OPEN_PATH = StringUtils.joinPaths(this.channelsPrefix, 'openPath');

    // 计算文件大小
    static GET_FOLDER_SIZE = StringUtils.joinPaths(this.channelsPrefix, 'getFolderSize');
